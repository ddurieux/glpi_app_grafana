      ...
      if (response.status === 200) {
        ...
        for (var q of queryTargets) {
          target_pool.push(function(targetargs) {
            ...
            var to = function (bksrv, urloptions, timeperiods, alltargetresult) {
              return bksrv.datasourceRequest(urloptions).then(response => {
                if (response.status >= 200 && response.status < 300) {
                  ...
                  var resultfunc = this.promiseMergeTargetResult(timeperiods, field_num, q);
